...
internal final class IsBlankBinaryFloatingPointTypesTests: BaseAssertionsTests {
    ///
    func testIsBlankForZero() {
        self.assertMustBeValid(assert(isBlank: 0.0))
    }

    ///
    func testIsBlankForPositive() {
        self.assertMustBeNotValid(assert(isBlank: 0.1))
    }
    ...
    func testIsNotBlankForZero() {
        self.assertMustBeNotValid(assert(isNotBlank: 0.0))
    }

    ///
    func testIsNotBlankForPositive() {
        self.assertMustBeValid(assert(isNotBlank: 0.1))
    }
    ...
    func testIsBlankCustomMessage() {
        ...
        self.assertMustBeNotValid(
            assert(isBlank: 0.1, orNotification: message),
            withMessage: message
        )
    }
    ...
    func testIsNotBlankCustomMessage() {
        ...
        self.assertMustBeNotValid(
            assert(isNotBlank: 0.0, orNotification: message),
            withMessage: message
        )
    }
}
